...
interface Props {
  ...
  link: string | null;
  /** Só os ícones pequenos (display no cabeçalho), sem card nem edição. */
  compact?: boolean;
}
...
export default function ProfessionalSocialLinks({ proId, isOwner, isPaidPlan, whatsapp, instagram, link, compact = false }: Props) {
  ...
  // Modo compacto: ícones pequenos pro cabeçalho (sem card/edição)
  if (compact) {
    if (!isPaidPlan || !hasAny) return null;
    const c = "w-8 h-8 rounded-full flex items-center justify-center text-white active:scale-95 transition-transform shadow-sm";
    return (
      <div className="flex items-center gap-1.5">
        {waUrl && <a href={waUrl} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className={c} style={{ background: "#25D366" }}><WhatsappIcon className="w-4 h-4" /></a>}
        {igUrl && <a href={igUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className={c} style={{ background: "linear-gradient(45deg,#f09433,#dc2743,#bc1888)" }}><InstagramIcon className="w-4 h-4" /></a>}
        {otherUrl && <a href={otherUrl} target="_blank" rel="noopener noreferrer" aria-label="Link" className={c} style={{ background: "#525252" }}><Link2 className="w-4 h-4" /></a>}
      </div>
    );
  }
