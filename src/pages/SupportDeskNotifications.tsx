import SupportDeskLayout from "@/components/SupportDeskLayout";
...
const SupportDeskNotifications = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("support-desk-notifications")
  ...
  const handleClick = (n: Notification) => {
    if (n.link && n.link.startsWith("/suporte-desk")) {
      navigate(n.link);
    } else if (n.link) {
      navigate(n.link);
    }
  ...
  return (
    <SupportDeskLayout>
      <div className="flex items-center gap-3 mb-4">
        <Link to="/suporte-desk" className="p-1.5 rounded-lg hover:bg-muted transition-colors">
...
export default SupportDeskNotifications;
