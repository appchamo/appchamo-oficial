...
const Search = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (user) {
        const { data: profile } = await supabase.from("profiles").select("address_city").eq("user_id", user.id).single();
  ...
  // Carrega profissões quando muda a categoria
  useEffect(() => {
    if (filterCategory) {
  ...
  const filtered = pros.filter(p => {
    if (search && !fuzzyMatch(search, `${p.full_name} ${p.category_name}`)) return false;
            ...
            <SheetContent side="bottom" className="rounded-t-3xl h-[85vh] overflow-y-auto">
              ...
              <div className="py-6 space-y-6">
                ...
                <div className="space-y-3">
                  <label className="text-sm font-bold flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" /> Localização e Raio</label>
                  <input 
                    value={filterCity} onChange={(e) => setFilterCity(e.target.value)}
                    placeholder="Cidade" className="w-full p-3 rounded-xl border bg-background text-sm mb-2" 
                  />
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {filterCategory && (
                    <div>
                      <label className="text-sm font-bold mb-2 block">Profissão</label>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button 
                        ...
                        className={`p-2 rounded-lg border transition-all ${filterMinRating >= star ? "bg-primary/10 border-primary" : "bg-card"}`}
                      >
                        <Star className={`w-6 h-6 ${filterMinRating >= star ? "fill-primary text-primary" : "text-muted"}`} />
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="flex items-center justify-between p-4 bg-muted/30 rounded-2xl">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold">Apenas Empresas</span>
                    </div>
                    <Switch checked={filterCompaniesOnly} onCheckedChange={setFilterCompaniesOnly} />
                  </div>
                </div>

                <button onClick={() => { setFilterCategory(""); setFilterMinRating(0); setFilterVerifiedOnly(false); setFilterCompaniesOnly(false); }} className="w-full py-3 text-sm font-bold text-primary">Limpar Filtros</button>
              </div>
            </SheetContent>
          </Sheet>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filtered.map((pro) => (
              <Link key={pro.id} to={`/professional/${pro.id}`} className="flex items-center gap-3 bg-card border rounded-2xl p-4 hover:border-primary/30 transition-all group">
                <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center overflow-hidden border-2 border-background shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground truncate">{pro.category_name} {pro.profession_name && `· ${pro.profession_name}`}</p>
