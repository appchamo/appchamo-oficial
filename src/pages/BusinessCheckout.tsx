      ...
      <main className="max-w-md mx-auto px-4 py-5 pb-20">
        ...
        <header className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2 py-0.5 rounded-full bg-violet-100 text-violet-600 text-[10px] font-bold uppercase tracking-wider">Checkout</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground">Assinar Plano Business</h1>
          <p className="text-sm text-muted-foreground">Complete os dados para ativar sua empresa</p>
        </header>
        ...
        <div className="space-y-4">
          ...
          <section className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-muted-foreground uppercase ml-1">CNPJ *</label>
                <input 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-muted-foreground uppercase ml-1">CEP *</label>
                  <div className="relative">
                    ...
                    />
                    {searchingCep && <Clock className="w-4 h-4 animate-spin absolute right-3 top-3.5 text-violet-500" />}
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-muted-foreground uppercase ml-1">Nº *</label>
                  <input 
              ...
              {businessData.street && (
                <div className="p-3 bg-muted/40 rounded-xl border border-dashed border-border animate-in fade-in">
                  <p className="text-xs font-semibold">{businessData.street}</p>
                  <p className="text-[10px] text-muted-foreground">{businessData.neighborhood} - {businessData.city}/{businessData.state}</p>
                </div>
              )}

              <div className="pt-2">
                <label className="text-[11px] font-bold text-muted-foreground uppercase ml-1 mb-1 block">Comprovante de CNPJ *</label>
                <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-6 cursor-pointer transition-all ${proofFile ? 'border-emerald-500 bg-emerald-500/5' : 'border-border hover:bg-muted/50'}`}>
                  {proofFile ? <FileText className="w-6 h-6 text-emerald-600" /> : <Upload className="w-6 h-6 text-violet-500" />}
          ...
          <section className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
            <header className="flex items-center justify-between mb-1">
              ...
              <span className="text-sm font-bold text-violet-600">R$ 250,00</span>
            </header>
            ...
            <div className="space-y-3">
              <input 
                ...
                className="w-full p-3.5 border rounded-xl bg-background outline-none focus:ring-2 focus:ring-violet-500/20 text-sm uppercase" 
              />
              <input 
          ...
          <button 
            ...
            disabled={loading}
            className="w-full py-4 bg-violet-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-violet-200 active:scale-95 transition-all disabled:opacity-50"
          >
            {loading ? <Loader2 className="animate-spin w-5 h-5" /> : <>Finalizar Assinatura <ChevronRight className="w-4 h-4" /></>}
          </button>
